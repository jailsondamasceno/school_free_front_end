<template>
  <div>
    ...
    <div v-if="carregou ===true">
      <PageTitle icon="fa fa-money" :main="despesas_cad"/>
      <div class="botoes">
        <BotaoCadastrar
          class="btn-cad"
          v-if="SU_AD_DI()"
          cadastro="cadastro-despesa"
          titulo="Cadastrar Despesa:"
        />
        <b-button variant="primary" @click="buscarDespesas()">
          Nova Busca
...
export default {
  ...
  data() {
    return {
      ...
      categorias: [],
      sub_categorias: [],
      despesas_cad: "Despesas",
      despesas: [],
      status: ["A PAGAR", "PAGO", "EM ATRASO"],
      fields: [
        { key: "nome", label: "Despesa", sortable: true },
        { key: "data_lancamento", label: "Lançada Em", sortable: true },
  ...
  methods: {
    ...
    setaUrl() {
      ...
      if (perfil_usuario === "SUPER_ADMIN" || perfil_usuario === "ADMIN") {
        return `${baseApiUrl}/relatoriosFinanceiros/despesasComFiltro/${
    ...
    buscarDespesas() {
      ...
      axios
        ...
        .then(res => {
          ...
          this.despesas.despesas.map(item => {
    ...
    buscarCategorias() {
      const url = `${baseApiUrl}/categoriasDespesas/todasInstituicao/${
        this.usuario.id_instituicao
      }`;
      axios
        .get(url)
    ...
    buscarSubcategoria() {
      setTimeout(() => {
        ...
        const url = `${baseApiUrl}/categoriasDespesas/subcategorias/todasCategoria/${
          this.usuario.id_instituicao
