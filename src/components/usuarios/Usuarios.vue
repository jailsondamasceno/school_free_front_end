<template>
  <div>
    <div class="usuarios">
      <PageTitle icon="fa fa-users" :main="usuarios_cad" sub="Cadastrados"/>
      <BotaoCadastrar v-if="SU_AD_DI()" cadastro="cadastro-usuario" titulo="Cadastrar Usuário: "/>
    </div>
    <hr>
    <b-container fluid>
      <b-row>
      ...
      >
        ...
        <form>
          ...
          <b-row class="turno-modal">
            ...
            <b-col md="6" sm="12">
              <p>Imagem perfil:</p>
              <b-form-input type="url" placeholder="Cole o link da imagem:" v-model="user.imagem_perfil"></b-form-input>
            </b-col>
          </b-row>
...
export default {
  name: "Usuarios",
  components: { PageTitle, BotaoCadastrar, Datepicker },
  data() {
    return {
      pt: ptBR,
      idiomas: ["PORTUGUÊS", "INGLÊS"],
      status: ["ATIVO", "INATIVO"],
      usuarios_cad: "Usuários",
      users: Array,
      escolas: [],
      instituicoes: [],
      user: {},
      ...
      fields: [
        ...
        { key: "perfil_atual", label: "Perfil", sortable: true },
  ...
  methods: {
    ...
    ativa_inativa(usuario, acao) {
      const url = `${baseApiUrl}/usuarios/atualizar/${this.usuario.id_instituicao}/${usuario.id}`;
      axios
    ...
    SU_AD_DI(){
      const perfil_usuario = this.$store.state.usuario.perfil_atual;
      if(perfil_usuario==='SUPER_ADMIN'|| perfil_usuario==='ADMIN'||perfil_usuario==='DIRETOR'){
          return true
      }else{return false}
    },
    formatarData(data) {
